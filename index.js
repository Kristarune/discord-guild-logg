...
if (!token) throw new Error("Missing DISCORD_TOKEN secret."); if (!allowedGuildId) throw new Error("Missing GUILD_ID secret.");
...
client.once(Events.ClientReady, (readyClient) => { console.log(Ready as ${readyClient.user.tag}); console.log(Only logging guild ID: ${allowedGuildId}); });
...
return; } const channel = interaction.options.getChannel("channel", true); if (!channel.isTextBased()) { await interaction.reply({ content: "Choose a normal text channel.", ephemeral: true, }); return; } setLogChannelId(channel.id); await interaction.reply({ content: `Logging is now set to ${channel}.`, ephemeral: true, }); return; 

}

if (interaction.isChatInputCommand()) { const options = interaction.options.data .map((option) => { const value = option.value ?? "None"; return **${option.name}:** ${value}; }) .join("\n");
...
if (!logChannelId) return; if (message.channelId === logChannelId) return; if (!message.author.bot) return;
